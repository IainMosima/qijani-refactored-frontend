...
export async function getOrders(token: string): Promise<OrderStructure[]> {
    const response = await fetchData('/api/v1/orders/', { headers: { 'Authorization': 'Bearer ' + token } ,method: 'GET' });
...
export async function cancelOrder (orderId: string, token: string) {
    const response = await fetchData(`/api/v1/orders/${orderId}`,{headers: { 'Authorization': 'Bearer ' + token }, method: 'DELETE'});
    return response;
}