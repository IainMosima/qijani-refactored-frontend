...
const Checkout = ({ loggedInUser }: CheckoutProps) => {
    ...
    useEffect(() => {
      async function fetchPackageInfo(packageId: string) {
        ...
        if(response && response.items) {
            ...
            const totalPrice = response.items?.reduce((total, item) => {
                if (item.price) {
    ...
    return ( 
        <div className="app__checkout">
            ...
            {packageInfo &&
                <div>
                    <h2 className="package-name">{packageName.current} ({items?.length} items)</h2>
            ...
            <div className="footer-price">
                <div className="price-card">
                    <h3>Subtotal</h3>
                    <h3 className="price">Ksh {total * .8}</h3>
                    
                    <h3>VAT</h3>
                    <h3 className="price">Ksh {total * .2}</h3>
